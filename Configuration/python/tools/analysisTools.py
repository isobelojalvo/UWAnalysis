...
def defaultReconstruction(process,triggerProcess = 'HLT',triggerPaths = ['HLT_Mu9','HLT_Mu11_PFTau15_v1','HLT_Mu11_PFTau15_v1','HLT_Mu11_PFTau15_v2','HLT_Mu15_v1','HLT_Mu15_v2']):
  ...
  switchJetCollection(process,cms.InputTag('ak5PFJets'),True,True,('AK5PF',['L1FastJet','L2Relative','L3Absolute','L2L3Residual']),False,cms.InputTag("ak5GenJets"),True,"ak5","",None,[])
...
def defaultReconstructionMC(process,triggerProcess = 'HLT',triggerPaths = ['HLT_Mu9'],calibrateMET = False,calibrationScheme = "BothLegs"):
  ...
  #Use PF Jets
  switchJetCollection(process,cms.InputTag('ak5PFJets'),True,True,('AK5PF',['L1FastJet','L2Relative','L3Absolute']),False,cms.InputTag("ak5GenJets"),True,"ak5","",None,[])
...
def applyDefaultSelections(process):
  ...
  process.selectedPatJetsSyst.cut =  cms.string("abs(eta)<4.5&&pt>10&&neutralHadronEnergyFraction<0.99&&neutralEmEnergyFraction<0.99&nConstituents>1&&((abs(eta)>2.4)||(abs(eta)<2.4&&chargedHadronEnergyFraction>0&&chargedMultiplicity>0&&chargedEmEnergyFraction<0.99))")
...
def switchToMuPFIsolation(process,muons):
  ...
  process.muPFIsoValueAll = cms.EDProducer("CandIsolatorFromDeposits",
       deposits = cms.VPSet(
          ...
          weight = cms.string('1'),
          vetos = cms.vstring('0.001','Threshold(1.0)'),
  ...
  process.muPFIsoValuePU = cms.EDProducer("CandIsolatorFromDeposits",
     deposits = cms.VPSet(
         cms.PSet(
             ...
             deltaR = cms.double(0.4),
             weight = cms.string('1'),
             vetos = cms.vstring('0.0000','Threshold(1.0)'),
             skipDefaultVeto = cms.bool(True),
             mode = cms.string('sum')
         ) 
     )
