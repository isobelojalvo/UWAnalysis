...
ETanalysisConfigurator.addSelector('osDiElectrons','PATElePairSelector','abs(leg1.eta())<2.5&&abs(leg2.eta())<2.5&&abs(leg1.userFloat("ipDXY"))<0.045&&abs(leg2.userFloat("ipDXY"))<0.045&&abs(leg1.userFloat("dz"))<0.2&&abs(leg2.userFloat("dz"))<0.2&&leg1.pt()>15&&leg2.pt()>15&&charge==0&&leg2.userFloat("wp95V")>0&&leg1.userFloat("wp95V")>0&&(leg1.chargedHadronIso()+max(leg1.userIso(1)+leg1.neutralHadronIso()-0.5*leg1.userIso(2),0.0))/leg1.pt()<0.3&&(leg2.chargedHadronIso()+max(leg2.userIso(1)+leg2.neutralHadronIso()-0.5*leg2.userIso(2),0.0))/leg2.pt()<0.3','ZEEVEto',0,100)
