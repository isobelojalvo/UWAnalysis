...
int main (int argc, char* argv[]) 
{
	...
	parser.addOption("chan",optutl::CommandLineParser::kString,"Channel  ","mutau");
	...
	parser.addOption("btagRelaxedSelection",optutl::CommandLineParser::kString,"1 bTag Relaxed Selection","");
	...
	BkgOutput output = creator.runOSLSMT(parser.stringValue("preselection"),"_inclusive",parser.stringValue("zEmbeddedSample"),parser.doubleValue("topSF"));
        ...
        //input preselction twice (is okay)
	...
	if(bitmask[0]==1){	 
		...
		BkgOutput outputMSSM2btag = creator.runFullExtrapBtag(parser.stringValue("btagRelaxedSelection2"),parser.stringValue("preselection"),MSSM2,"_2btag",parser.stringValue("zEmbeddedSample"),parser.doubleValue("topSF"),
				1,//parser.doubleValue("zExtrap"),
				1,//parser.doubleValue("zExtrapErr"),
				bTagSF2
				);
