#!/bin/sh
mkdir /nfs_scratch/$USER/htt_weighted11
cp /nfs_scratch/$USER/htt_unweighted11/* /nfs_scratch/$USER/htt_weighted11/.
cd /nfs_scratch/$USER/htt_weighted11/
...
if [ $weightZ -eq 1 ]
    then
    EventWeightsIterativeGen outputFile='Z1Jets.root'     weight=6025.2    histoName='MT/results' sumHistoName='sumweights/genWeights'
...
if [ $weight -eq 1 ]
    ...
    #EventWeightsIterativeGen outputFile='ZJets_ext.root'     weight=6025.2    histoName='MT/results' sumHistoName='sumweights/genWeights' #Z in in Z
    ...
    hadd -f DiBoson.root WZJets.root WZTo*root ZZTo*.root St_top.root t*_tW.root
