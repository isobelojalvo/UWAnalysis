makeTFile * makeBandFromText(TString input,TString name) {
...
void formatGraph95(TGraphAsymmErrors * b68) {
  b68->SetTitle("");
...
void formatGraph68(TGraphAsymmErrors * b95) {
  b95->SetTitle("");
...
void addBand(TCanvas *c ,TDirectory *f , TString name,TLegend* legend) {
  ...
  TGraphAsymmErrors  *bL68 = b68->Clone();
  formatGraph68(b68);
  b68->Draw("3same");

  bL68->SetLineColor(kRed);
  bL68->SetLineStyle(9);
  bL68->SetLineWidth(4);
  bL68->Draw("LXsame");
  ...
  if(legend!=0) {
    legend->AddEntry(bL68,"Median Expected  CLs","l");
    legend->AddEntry(b68,"#pm 1 #sigma Expected  CLs","f");
    legend->AddEntry(b95,"#pm 2 #sigma Expected  CLs","f");
  }
} 
...
TCanvas * makeExpectedVsObservedComp(TString cls,TString bayes, TString name,bool log = true) {

  TFile *f1 = new TFile(cls);
  TFile *f2 = new TFile(bayes);
...
TCanvas * makeExpectedVsObservedSM(TDirectory *f, TString name,bool log = false) {
  ...
  TCanvas *c = new TCanvas("c_"+name+"ExpVsObj","c_"+name+"ExpVsObj",800,500);
  c->cd();
  TH1F *frame = c->DrawFrame(110.,0.5,145.,10.);
  frame->GetXaxis()->SetTitle("m_{H} [GeV]");
  frame->GetYaxis()->SetTitle("#sigma_{95% CL} /#sigma_{SM}");
  ...
  TLegend *l = addLegend(c,0.2,0.6,0.6,0.8);
  addBand(c ,f ,name,l) ;
  addLine(c ,f ,name+"_obs",kBlack,l,"Observed CLs",true);
  l->Draw();
  if(log)
    c->SetLogy();
  c->RedrawAxis();
  ...
  TLatex *a = new TLatex(112.,7.5,"CMS 2011, 4.7 fb^{-1}");
  TLatex *b = new TLatex(112.,7.,"95% CL Limits");
  b->SetTextSize(0.04);
  a->Draw();
  b->Draw();
...
TCanvas * makeExpectedVsObserved(TDirectory *f, TString name,bool log = false) {
  ...
  frame->GetYaxis()->SetTitleOffset(0.85);
  frame->Draw();
  TLegend *l = addLegend(c,0.6,0.6,0.9,0.85);
  addBand(c ,f ,name,l) ;
  addLine(c ,f ,name+"_obs",kBlack,l,"Observed CLs",true);
  l->Draw();
  if(log)
    c->SetLogy();
  c->RedrawAxis();
  TLatex *a = new TLatex(95.,290,"CMS Preliminary, 4.6 fb^{-1}");
  TLatex *b = new TLatex(95.,280.,"95% CL Limits");
  b->SetTextSize(0.04);
  a->Draw();
  b->Draw();
...
void makeLimitsSM(TString file,TString which)
{
  ...
  gROOT->ProcessLine(".x UWAnalysis/ROOT/interactive/loadRooInteractiveLOCAL.C");
  gROOT->ProcessLine(".L UWAnalysis/ROOT/interactive/bandUtils.cxx+");
  TFile * bands = TFile::Open("bands"+which+".root","UPDATE");
