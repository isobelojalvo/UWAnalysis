...
int main (int argc, char* argv[]) 
{
   ...
   parser.addOption("WJets",optutl::CommandLineParser::kInteger,"WJETS",0);
...
void readdir(TDirectory *dir, optutl::CommandLineParser parser, char TreeToUse[]) 
{
  ...
  while ((key = (TKey*)next())) {
    ...
    if (obj->IsA()->InheritsFrom(TDirectory::Class())) {
      ...
      float Wweight = 1.0;
      float WweightUp = 1.0;
      float WweightDown = 1.0;

      TBranch *newBranch = t->Branch(parser.stringValue("branch").c_str(),&weight,(parser.stringValue("branch")+"/F").c_str());
      TBranch *WJetsBranch = t->Branch("WWeight",&Wweight,"WWeight/F");
      TBranch *WJetsBranchUp = t->Branch("WWeightUp",&WweightUp,"__WEIGHTUp__/F");
      TBranch *WJetsBranchDown = t->Branch("WWeightDown",&WweightDown,"__WEIGHTDown__/F");
      ...
      for(Int_t i=0;i<t->GetEntries();++i)
	{
	  ...
	  if(std::string(TreeToUse).find("muTauEventTree")!= std::string::npos){
	    ...
	    Wweight=1.15743-(0.00736136)*pt2+(4.3699e-05)*pt2*pt2-(1.188e-07)*pt2*pt2*pt2;
	    //std::cout<<"Wweight "<<Wweight<<" __WEIGHT__ " <<weight << " New W Weight "<<weight*Wweight<<std::endl;
	    WweightUp  = Wweight+0.50*(1.0 - Wweight) ;
	    WweightDown= Wweight-0.50*(1.0 - Wweight);
