...
int main (int argc, char* argv[]) 
{
   ...
   parser.addOption("combined",optutl::CommandLineParser::kInteger,"combined",0);
   parser.addOption("semilepton",optutl::CommandLineParser::kBool,"semilepton",false);
   parser.addOption("dilepton",optutl::CommandLineParser::kBool,"dilepton",false);
   parser.addOption("topWeightOnly",optutl::CommandLineParser::kBool,"topWeightOnly",false);
...
void readdir(TDirectory *dir, optutl::CommandLineParser parser, char TreeToUse[]) 
{
  ...
  char keyName[80]="no";
  bool topWeightOnly = parser.boolValue("topWeightOnly");  
  bool semilepton = parser.boolValue("semilepton");
  bool dilepton   = parser.boolValue("dilepton");
  ...
  while ((key = (TKey*)next())) {
    ...
    if (obj->IsA()->InheritsFrom(TDirectory::Class())) {
      ...
      if(!topWeightOnly)
	newBranch = t->Branch(parser.stringValue("branch").c_str(),&weight,(parser.stringValue("branch")+"/F").c_str());

      TBranch *TWeightBranch = t->Branch("TWeight",&Tweight,"TWeight/F");
	...
	a=0.148;b=-0.00129;
      ...
      else if(semilepton&&dilepton){
	std::cout<<"Both Semi Lepton and Di Lepton set; defaulting to combined SF"<<std::endl;
	a=0.156;b=-0.00137;
      }
      else{
	a=0.156;b=-0.00137;
      ...
      t->SetBranchAddress("topGenPt",&topGenPt);
      t->SetBranchAddress("antiTopGenPt",&antiTopGenPt);
      ...
      for(Int_t i=0;i<t->GetEntries();++i)
	{
	  ...
	  //Top weighting done here
	  Tweight= sqrt(exp(a+b*topGenPt)*exp(a+b*antiTopGenPt));
	  if(topGenPt == -1 || antiTopGenPt == -1)
	    Tweight = 1;

	  //If we are NOT doing just top weighting then also calculate efficiency weights
	  if(!topWeightOnly){
