...
public:
  ...
  /*
  bool isbtagged(Float_t pt, Float_t eta, Float_t csv, Int_t jetflavor, Bool_t isdata, UInt_t btagsys, UInt_t mistagsys, Bool_t is2012);
  ...
  */
  ...
  Bool_t isbtagged(Float_t pt, Float_t eta, Float_t csv, Int_t jetflavor, Bool_t isdata, UInt_t btagsys, UInt_t mistagsys, Bool_t is2012)
  {
    ...
    TRandom3 * randm1;
    randm1 = new TRandom3(12345);
    randm1->SetSeed((int)((eta+5)*100000));
  ...
  if(fabs(jetflavor) == 5) {                // real b-jet
    if(csv > 0.679) {                       // if tagged
      ...
      btagged = kFALSE;
      if(promoteProb_btag > 0 && randm1->Uniform() < promoteProb_btag) btagged = kTRUE;  // promote it to tagged
  ...
  if(csv > 0.679) {         // if tagged
    ...
    btagged = kFALSE;
    if(promoteProb_mistag > 0 && randm1->Uniform() < promoteProb_mistag) btagged = kTRUE;  // promote it to tagged
    else                                                                btagged = kFALSE; // leave it untagged
  }
