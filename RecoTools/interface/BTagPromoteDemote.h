...
class BtagSFV
{
	public:
		...
		bool isbtaggeddown(Float_t pt, Float_t eta, Float_t csv, Int_t jetflavor, Bool_t isdata, UInt_t btagsys, UInt_t mistagsys, Bool_t is2015)
		{
			...
			if (fabs(jetflavor) == 5) {                // real b-jet
				SF = btag_reader_down.eval(BTagEntry::FLAV_B, eta, pt );
				eff =0.682165; 
			}
			else if (fabs(jetflavor) == 4) { 
				SF = btag_reader_down.eval(BTagEntry::FLAV_C, eta, pt );
				eff =0.180186;
			}  
			else {
				SF = btag_reader_light_down.eval(BTagEntry::FLAV_UDSG, eta, pt );
				eff =0.012357;
			}
		...
		bool isbtaggedup(Float_t pt, Float_t eta, Float_t csv, Int_t jetflavor, Bool_t isdata, UInt_t btagsys, UInt_t mistagsys, Bool_t is2015)
		{
			//btagged set to false
			Bool_t btagged = kFALSE;
			...
			if (fabs(jetflavor) == 5) {                // real b-jet
				SF = btag_reader_up.eval(BTagEntry::FLAV_B, eta, pt );
				eff =0.682165; 
			}
			else if (fabs(jetflavor) == 4) { 
				SF = btag_reader_up.eval(BTagEntry::FLAV_C, eta, pt );
				eff =0.180186;
			}  
			else {
				SF = btag_reader_light_up.eval(BTagEntry::FLAV_UDSG, eta, pt );
				eff =0.012357;
			}
